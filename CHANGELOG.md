...
- 🔄 **Transición de mount del bloque de penales** (`src/index.css:610-629` + `PredictionSlide.tsx:263` + `MatchCard.tsx:955`): cuando el usuario pasa de 1-0 a 1-1 con el stepper, el bloque de penales ahora aparece con slide-down (`translateY(-8px) → 0`, 240ms `ease-out`) + fade-in (`opacity 0 → 1`) en lugar del mount/unmount instantáneo. Aplicado en paridad a modal y cards inline del dashboard. `prefers-reduced-motion` desactiva la animación. **Limitación aceptada**: el unmount sigue siendo instantáneo (no hay exit animation); si en el futuro se nota, se puede pasar a un patrón `useState` con delay de unmount.
...
- 🔧 Nuevo keyframe `penaltyBlockEnter` (transform + opacity, 240ms `ease-out`, `fill-mode: both`) + utility `.animate-penalty-enter` en `@layer components` (`src/index.css:610-629`). Override en `@media (prefers-reduced-motion: reduce)` (línea 880) agrupado con `.animate-pulse-soft`.
...
- 🔧 Decisión: transición de mount CSS-only, sin estado React nuevo, sin librerías externas, sin componente `<PenaltySelector>` compartido. La duplicación de 1 palabra (`animate-penalty-enter`) entre los 2 archivos se aceptó por simplicidad (overhead de abstracción mayor que el beneficio).
- 🔧 Accesibilidad: badge con `role="status"` + `aria-live="polite"`, icono decorativo con `aria-hidden="true"`.
- 🧪 **Fix de tests flaky en `PositionsView.test.tsx`** (commit `6aca6f0`): 2 tests usaban `getByRole` / `getByText` síncronos para elementos que renderizan async ("Cargando bracket..." → "Grupo A"). Cambiados a `findByRole` / `findByText` (async). Los 6 tests "fallidos" sobre `isFeatureEnabled("BRACKET_V2")` reportados inicialmente eran flaky tests (condiciones de carrera en workers de Vitest), no un bug real — quedaron en 726/726 passing.
